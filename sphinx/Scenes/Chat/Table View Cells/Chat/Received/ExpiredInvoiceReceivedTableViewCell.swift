...
class ExpiredInvoiceReceivedTableViewCell: ExpiredInvoiceCommonChatTableViewCell, MessageRowProtocol {
    ...
    func configureMessageRow(messageRow: TransactionMessageRow, contact: UserContact?, chat: Chat?) {
        ...
        bubbleView.showIncomingExpiredInvoiceBubble(messageRow: messageRow, bubbleWidth: bubbleWidth)

        let amountString = messageRow.getAmountString()
        amountLabel.text = "\(amountString)"

        drawDiagonalLine(lineContainer: expiredInvoiceLine, incoming: true)
