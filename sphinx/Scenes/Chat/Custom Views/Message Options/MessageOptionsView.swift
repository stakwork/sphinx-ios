...
extension MessageOptionsView : MessageOptionViewDelegate {
    ...
    private func handleActionWith(_ tag: Int) {
        ...
        case .CopyLink:
            ClipboardHelper.copyToClipboard(text: message.messageContent?.replacingHightlightedChars.stringFirstLink ?? "", message: "link.copied.clipboard".localized)
        case .CopyPubKey:
            ClipboardHelper.copyToClipboard(text: message.messageContent?.replacingHightlightedChars.stringFirstPubKey?.0 ?? "", message: "pub.key.copied.clipboard".localized)
        case .CopyCallLink:
