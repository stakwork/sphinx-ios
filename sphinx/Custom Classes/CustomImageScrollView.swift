...
@objc public protocol CustomImageScrollViewDelegate: UIScrollViewDelegate {
    func imageScrollViewDidChangeOrientation(imageScrollView: CustomImageScrollView)
}

open class CustomImageScrollView: UIScrollView {
    ...
    @objc public private(set) var zoomView: SDAnimatedImageView? = nil
    
    @objc open weak var imageScrollViewDelegate: CustomImageScrollViewDelegate?
    ...
    private func initialize() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(CustomImageScrollView.changeOrientationNotification), name: UIDevice.orientationDidChangeNotification, object: nil)
    }
    ...
    @objc open func display(image: UIImage) {
        ...
        zoomView = SDAnimatedImageView(image: image)
        zoomView!.isUserInteractionEnabled = true
        addSubview(zoomView!)
        
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(CustomImageScrollView.doubleTapGestureRecognizer(_:)))
    ...
    @objc open func display(gif: SDAnimatedImage) {
        if let zoomView = zoomView {
            zoomView.removeFromSuperview()
        }
        
        zoomView = SDAnimatedImageView(image: gif)
        zoomView!.isUserInteractionEnabled = true
        addSubview(zoomView!)
        ...
        configureImageForSize(gif.size)
    ...
    @objc func doubleTapGestureRecognizer(_ gestureRecognizer: UIGestureRecognizer) {
        ...
        if zoomScale >= maximumZoomScale / 2.0 {
            ...
            let zoomRect = zoomRectForScale(CustomImageScrollView.kZoomInFactorFromMinWhenDoubleTap * minimumZoomScale, center: center)
...
extension CustomImageScrollView: UIScrollViewDelegate {
